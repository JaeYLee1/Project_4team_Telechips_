...
ModuleBPage::ModuleBPage(QWidget *parent)
    : QWidget(parent),
      ...
      m_powerProtectionCard(new QFrame(this)),
      m_powerStateLabel(new QLabel("POWER NORMAL", this)),
      m_powerTotalLabel(new QLabel("TOTAL : 0.00 W", this)),
      m_powerLimitLabel(new QLabel("LIMIT : 0.00 W", this)),
      m_powerWarningLabel(new QLabel("WARNING : 0 / 4", this)),
      ...
      m_targetTempC(15)
{
    ...
    QLabel *coolingTitle = new QLabel("REFRIGERATION STATUS", this);
    ...
    QLabel *tempDesc = new QLabel(
        "Target temperature is initialized to 15°C when Module B becomes active.\n"
    ...
    // ============================================================
    m_powerProtectionCard->setObjectName("PowerProtectionCard");
    m_powerProtectionCard->setMinimumHeight(132);
    m_powerProtectionCard->setStyleSheet(
        "QFrame#PowerProtectionCard {"
        " background: #101F2E;"
        " border: 1px solid #2C465D;"
        " border-radius: 18px;"
        "}"
    ...
    QVBoxLayout *powerLayout = new QVBoxLayout(m_powerProtectionCard);
    powerLayout->setContentsMargins(18, 16, 18, 16);
    powerLayout->setSpacing(8);

    QLabel *powerTitle = new QLabel("POWER PROTECTION", m_powerProtectionCard);
    ...
    m_powerStateLabel->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    m_powerStateLabel->setStyleSheet(
        "font-size: 22px;"
        "font-weight: 900;"
        "color: #20E0B5;"
        "background: transparent;"
    ...
    m_powerTotalLabel->setStyleSheet(powerInfoStyle);
    m_powerLimitLabel->setStyleSheet(powerInfoStyle);
    m_powerWarningLabel->setStyleSheet(powerInfoStyle);

    QHBoxLayout *powerValueRow = new QHBoxLayout();
    powerValueRow->setSpacing(16);
    powerValueRow->addWidget(m_powerTotalLabel);
    powerValueRow->addWidget(m_powerLimitLabel);
    powerValueRow->addWidget(m_powerWarningLabel);
    powerValueRow->addStretch();

    powerLayout->addWidget(powerTitle);
    powerLayout->addWidget(m_powerStateLabel);
    powerLayout->addLayout(powerValueRow);
    ...
    tempLayout->addWidget(m_powerProtectionCard);
                                  ...
                                  int fsmState)
{
    ...
    else
    {
        ...
        * 다음 장착 시 목표온도를 다시 15°C로 초기화하기 위함
        */
        m_targetInitialized = false;
    }
}
                                       ...
                                       int targetTempX10)
{
    ...
    /*
     * Module B ACTIVE 상태에서 최초 1회만 목표온도 15°C로 초기화
     */
...
void ModuleBPage::setPowerProtectionStatus(int powerWarningCount,
                                           int powerFault,
                                           int totalPowerMw,
                                           int powerLimitMw)
{
    ...
    m_powerTotalLabel->setText(
        QString("TOTAL : %1 W").arg(totalPowerW, 0, 'f', 2)
    );

    m_powerLimitLabel->setText(
        QString("LIMIT : %1 W").arg(limitPowerW, 0, 'f', 2)
    );

    if (coolingFault)
    {
        m_powerWarningLabel->setText("FAULT : LOW COOLING CURRENT");

        m_powerStateLabel->setText("COOLING FAULT");
        m_powerStateLabel->setStyleSheet(
            "font-size: 22px;"
            "font-weight: 900;"
            "color: #FF4D5A;"
    ...
    {
        m_powerWarningLabel->setText(
            QString("WARNING : %1 / 4").arg(powerWarningCount)
        );

        m_powerStateLabel->setText("POWER CUTOFF");
        m_powerStateLabel->setStyleSheet(
            "font-size: 22px;"
            "font-weight: 900;"
            "color: #FF4D5A;"
    ...
    {
        m_powerWarningLabel->setText("WARNING : 0 / 4");

        m_powerStateLabel->setText("POWER NORMAL");
        m_powerStateLabel->setStyleSheet(
            "font-size: 22px;"
            "font-weight: 900;"
            "color: #20E0B5;"
