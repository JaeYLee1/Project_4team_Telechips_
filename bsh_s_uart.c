...
static void update_shared_memory(const TelemetryData *data)
{
    ...
    g_shm_ptr->peltier_pwm    = (uint8_t)data->peltier_duty;
    ...
    /* ---- 추가: AI 프로세스가 쓴 sleep_flag / warning_flag 확인용 로그 ---- */
    printf("[SHM CHECK] sleep_flag=%u warning_flag=%u\n",
           g_shm_ptr->sleep_flag, g_shm_ptr->warning_flag);
    fflush(stdout);
    /* -------------------------------------------------------------------- */
...
static void check_ai_flags(void)
{
    ...
    sys_state    = g_shm_ptr->system_state;

    pthread_mutex_unlock(&g_shm_ptr->mutex);
    /* --- 임계 구역 탈출 (Unlock) --- */

    /* 출력은 lock 밖에서 (임계구역 최소화) */
    printf("[SHM CHECK] sleep_flag=%u warning_flag=%u system_state=%d\n",
           sleep_flag, warning_flag, sys_state);
    fflush(stdout);
}
